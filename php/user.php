...
if (isset($_POST['searchbtn'])) {
    ...
    $filter = ("SELECT * FROM real_estate_gallery NATURAL JOIN images_gallery where primary_or_secondary = '1' AND " . implode(" AND ", $queryParams));
    echo $filter;
    ...
    $announcesDATA = $conn->query("SELECT  a.*, i.image_path AS primary_image_path FROM real_estate_gallery a LEFT JOIN images_gallery i ON a.id = i.property_id AND i.primary_or_secondary = '1' LIMIT 8 OFFSET $StartIndex")->fetchAll(PDO::FETCH_ASSOC);

    $primaryImagePaths = array();
    foreach ($announcesDATA as $val) {
        $primaryImagePaths[$val['id']] = $val['primary_image_path'];
    }
...
$city = "SELECT DISTINCT City FROM real_estate_gallery";
...
if (isset($_POST['date_sort'])) {
    ...
    $sql = "SELECT * FROM real_estate_gallery NATURAL JOIN `images_gallery` where primary_or_secondary = '1' ORDER BY $sort_field $sort_order LIMIT 8 OFFSET $StartIndex";
    $stmt = $conn->query($sql);
    // Fetch the sorted cards
    $sorted = $stmt->fetchAll(PDO::FETCH_ASSOC);
  
    $page = 'SELECT * FROM real_estate_gallery';
                ...
                if ($_SERVER["REQUEST_METHOD"] == "GET") { //check if the method is get
                        ...
                        <div class="card ">
                            <div class="content">
                                <a href="details.php?id=<?php echo $val['id']; ?>">
                                    ...
                                    <img class="content-image" src="../<?php echo $primaryImagePaths[$val['id']]; ?>">
                    ...
                    for ($i = 0; $i < count($sorted); $i++) {
                        ?>
                            <div class="card ">
                                <div class="content">
                                    <a href="details.php?id=<?php echo $sorted[$i]['id']; ?>">
                                        <div class="content-overlay ">
